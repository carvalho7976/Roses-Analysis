...
def print_mean(df):
    ...
    """
    mean = df.groupby(['CONFIG'], as_index=False).agg({'RMSE': ['mean', 'std', 'max', 'min']})
...
if __name__ == '__main__':
    ...
    for dataset in DATASETS:
        df = pd.read_csv('resources/' + dataset +'-test-melo.csv', sep=";")
        #1C1 5C1 10C1 20C1 40C1
        #1C2 5C2 10C2 20C2 40C2
        #1C3 5C3 10C3 20C3 40C3
        #indexNames = df[ (df['CONFIG'] != '1C1') & (df['CONFIG'] != '1C2') & (df['CONFIG']!= '1C3') ].index
        #indexNames = df[ (df['CONFIG'] != '5C1') & (df['CONFIG'] != '5C2') & (df['CONFIG']!= '5C3') ].index
        #indexNames = df[ (df['CONFIG'] != '10C1') & (df['CONFIG'] != '10C2') & (df['CONFIG']!= '10C3') ].index
        #indexNames = df[ (df['CONFIG'] != '20C1') & (df['CONFIG'] != '20C2') & (df['CONFIG']!= '20C3') ].index
        #indexNames = df[ (df['CONFIG'] != '40C1') & (df['CONFIG'] != '40C2') & (df['CONFIG']!= '40C3') ].index

        #df.drop(indexNames , inplace=True)
        print(df.head(5))
        fig, ax = plt.subplots(figsize=(10,6))

        k = kruskal_wallis(df, 'RMSE', 'CONFIG')
